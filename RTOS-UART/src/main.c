...
static void task_execute(void *pvParameters) {
	...
	xQueueCommand = xQueueCreate(50, sizeof(char[64]));
	
	for (;;) {
		if (xQueueReceive(xQueueCommand, &buffer, (TickType_t)500 / portTICK_PERIOD_MS)) {
			
			if (strcmp(buffer, "led placa toggle") == 0) {
				pin_toggle(LED_PLACA_PIO, LED_PLACA_IDX_MASK);
				
			} else if (strcmp(buffer, "led placa on") == 0) {
				pio_clear(LED_PLACA_PIO, LED_PLACA_IDX_MASK);
				
			} else if (strcmp(buffer, "led placa off") == 0) {
				pio_set(LED_PLACA_PIO, LED_PLACA_IDX_MASK);
				
			} else if (strcmp(buffer, "led 1 toggle") == 0) {
			pin_toggle(LED1_PIO, LED1_IDX_MASK);
			
			} else if (strcmp(buffer, "led 1 on") == 0) {
			pio_clear(LED1_PIO, LED1_IDX_MASK);
			
			} else if (strcmp(buffer, "led 1 off") == 0) {
			pio_set(LED1_PIO, LED1_IDX_MASK);
			
			} else if (strcmp(buffer, "led 2 toggle") == 0) {
			...
			} else if (strcmp(buffer, "led 3 toggle") == 0) {
			pin_toggle(LED3_PIO, LED3_IDX_MASK);
			
			} else if (strcmp(buffer, "led 3 on") == 0) {
			pio_clear(LED3_PIO, LED3_IDX_MASK);
			
			} else if (strcmp(buffer, "led 3 off") == 0) {
...
static void task_led1(void *pvParameters)
{
	...
	const TickType_t xDelay = 3000 / portTICK_PERIOD_MS;
	const TickType_t xDelayL = 200 / portTICK_PERIOD_MS;
...
int main(void)
{
	...
	xQueueChar = xQueueCreate(50, sizeof(char));
	...
	xTaskCreate(task_uartRX, "uartRX", TASK_UARTRX_STACK_SIZE, NULL, TASK_UARTRX_STACK_PRIORITY, NULL);
	xTaskCreate(task_execute, "execute", TASK_EXECUTE_STACK_SIZE, NULL, TASK_EXECUTE_STACK_PRIORITY, NULL);
